...
describe("Zodiac Roles Module Direct SDK Execution", function () {
    let safeOwner: any;
    ...
    describe("Direct SDK Module Deployment", function () {
        it("should deploy and configure the Roles module using direct SDK", async function () {
            console.log("🚀 === DIRECT SDK MODULE DEPLOYMENT ===");

            // Step 1: Deploy module proxy using direct SDK approach
            ...
            // Create initialization data using the exact format expected by the Roles module
            // Based on the Zodiac SDK source code, the Roles module expects:
            // setUp(bytes calldata initParams) where initParams is (address owner, address avatar, address target)
            const initParams = ethers.utils.defaultAbiCoder.encode(
                ["address", "address", "address"],
                [safeOwner.address, safeAddress, safeAddress]
            );

            // Create the setUp function call with the correct selector
            ...
            // Deploy with explicit gas limit and higher value
            ...
            if (moduleProxyCreationEvent) {
                ...
                // Calculate expected address as fallback
                const expectedAddress = ethers.utils.getCreate2Address(
                    FLARE_ADDRESSES.factory,
                    ethers.utils.keccak256(
                        ethers.utils.defaultAbiCoder.encode(
                            ["address", "bytes", "uint256"],
                            [FLARE_ADDRESSES.roles, initData, saltNonce]
                        )
                    ),
                    ethers.utils.keccak256("0x3d602d80600a3d3981f3363d3d373d3d3d363d73" + FLARE_ADDRESSES.roles.slice(2) + "5af43d82803e903d91602b57fd5bf3")
                );
                moduleProxy = expectedAddress;
